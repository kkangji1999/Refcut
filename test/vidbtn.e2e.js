...
app.whenReady().then(async () => {
 try {
  ...
  console.log("\n=== 보관함에서 영상을 두 번 눌렀을 때 ===");
  const v = r.영상크게 || {};
  console.log("  크게 열렸나    " + (v.크게열림 ? "열렸다 · " + v.장수 + "장" : "안 열렸다"));
  console.log("  보관함         " + (v.보관함남음 ? "그대로 있다" : "닫혔다"));
  console.log("  이동 단추      " + (v.이동단추 === "none" ? "없다" : "있다"));
  if (!v.카드있나) 실패.push("보관함에 영상 카드가 없다 — 시험이 헐겁다");
  else {
    /* ★ 스틸은 그 자리에서 크게 보이는데 영상만 결과 화면으로 끌고 가면,
       보관함을 넘겨보다 갑자기 튕겨 나간다. 여기서 크게 보여야 한다. */
    if (!v.크게열림) 실패.push("보관함에서 영상을 두 번 눌러도 크게 보이지 않는다");
    if (!v.보관함남음) 실패.push("영상을 크게 보려는데 보관함이 닫혀 버린다");
    if (!(v.장수 > 1)) 실패.push(`크게 보기에 ${v.장수}장뿐이다 — 그 영상의 컷을 넘겨볼 수 있어야 한다`);
    if (v.이동단추 === "none") 실패.push("[이 컷으로 이동] 이 사라졌다 (결과 화면으로 가는 길은 남겨두기로 했다)");
  }
